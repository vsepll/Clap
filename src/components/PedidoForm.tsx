...
export default function PedidoForm({
  ...
  const handlePrev = () => { if (hasPrev && siblingPedidos && onNavigate) onNavigate(siblingPedidos[currentIndex - 1].id) }
  const handleNext = () => { if (hasNext && siblingPedidos && onNavigate) onNavigate(siblingPedidos[currentIndex + 1].id) }
    ...
    <>
      <Dialog open onOpenChange={open => !open && onCancel()}>
        {/*
          ...
          - p-0 gap-0 overflow-hidden: manual padding, no gap between grid rows
          - Mobile (<sm): bottom sheet — full-width, anchored to bottom, rounded top corners
          - Desktop (sm+): centered modal, max-w-2xl
        */}
        <DialogContent
          showCloseButton={false}
          className="p-0 gap-0 overflow-hidden sm:max-w-2xl
        ...
        >
          {/* Inner flex column — header (fixed) + body (scroll) + footer (fixed) */}
          <div className="flex flex-col max-h-[92svh] sm:max-h-[88svh]">
            ...
            <div className="shrink-0 flex items-center gap-3 px-5 pt-5 pb-4 border-b border-border">
              ...
              <DialogTitle className="flex-1 text-base font-medium leading-none">
            ...
            {/* ── Scrollable body ─────────────────────────────────────────── */}
            <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4" onPaste={handlePaste}>
              <div className="flex flex-col lg:flex-row gap-5">
                ...
                <div className="flex-1 space-y-4">
                        ...
                        <SelectValue placeholder="Seleccionar estado...">
                          {selectedGrupo && (
                            <span className="flex items-center gap-2">
                              <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: selectedGrupo.color }} />
                  ...
                  {/* Descripción + Notas — side by side on sm+ */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Textarea
                        ...
                        rows={2}
                        className="sm:min-h-[5rem] resize-none"
                      />
                    </div>
                    <div className="flex flex-col gap-1.5">
                  ...
                  {/* Financial + dates — 2 cols on mobile, 4 on sm+ */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="total">Total</Label>
                      <div className="relative">
                        <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">$</span>
                        <Input id="total" type="number" className="pl-6"
                          value={form.total.toString()} onChange={e => set('total', Number(e.target.value))} />
                      </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">$</span>
                        <Input id="sena" type="number" className="pl-6"
                          value={form.sena.toString()} onChange={e => set('sena', Number(e.target.value))} />
                      </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                  ...
                  {/* Saldo */}
                  <div className="flex items-center justify-between rounded-md border border-border bg-muted px-4 py-2.5">
                ...
                {/* Right column: images */}
                <div className="lg:w-44 shrink-0 flex flex-col gap-1.5">
                  ...
                  {form.imagenes.length === 0 ? (
                    <label className="min-h-[80px] lg:min-h-[140px] rounded-md border-2 border-dashed border-border bg-muted flex flex-col items-center justify-center cursor-pointer hover:border-ring/50 hover:bg-muted/80 transition-colors group">
                      <div className="flex flex-col items-center gap-1.5 text-muted-foreground group-hover:text-foreground transition-colors pointer-events-none">
                        <ImageIcon className="w-5 h-5" />
                        <span className="text-[10px] text-center leading-relaxed">Pegar o seleccionar</span>
                      </div>
                      <input type="file" accept="image/*" multiple onChange={handleFiles} className="sr-only" />
                    </label>
                  ) : (
                    /* Mobile: 4 small columns | Desktop: 2 columns */
                    <div className="grid grid-cols-4 lg:grid-cols-2 gap-1.5 max-h-[120px] lg:max-h-[240px] overflow-y-auto">
                      {form.imagenes.map((img, i) => (
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                            <Maximize2 className="w-3 h-3 text-white/80 pointer-events-none" />
                            <button
                              onClick={e => {
                                e.stopPropagation()
                                setForm(prev => ({ ...prev, imagenes: prev.imagenes.filter((_, j) => j !== i) }))
                              }}
                              className="absolute top-0.5 right-0.5 w-4 h-4 bg-red-500/90 hover:bg-red-500 rounded-full text-white flex items-center justify-center transition-colors"
                            >
            ...
            {/* ── Footer ─────────────────────────────────────────────────── */}
            <div className="shrink-0 border-t border-border bg-muted/50 rounded-b-xl max-sm:rounded-b-none px-4 py-3 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2">
              {formError && (
                <p className="sm:mr-auto text-xs text-red-400 text-center sm:text-left">{formError}</p>
              )}
              {editingPedido && (
                <Button variant="destructive" onClick={handleDelete} disabled={saving} size="sm" className="sm:size-default">
                  {showDeleteConfirm ? 'Confirmar eliminación' : 'Eliminar'}
                </Button>
              )}
              <Button variant="ghost" onClick={onCancel} disabled={saving} size="sm" className="sm:size-default">
                Cancelar
              </Button>
              <Button onClick={handleSubmit} disabled={saving} size="sm" className="sm:size-default">
      ...
      {lightboxIndex !== null && createPortal(
        ...
        >
          ...
          <div className="flex-1 flex items-center min-h-0 gap-2 sm:gap-4 px-2 sm:px-4" onClick={e => e.stopPropagation()}>
            <button
              onClick={() => setLightboxIndex(i => i !== null && i > 0 ? i - 1 : i)}
              disabled={lightboxIndex === 0}
              className="shrink-0 w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 disabled:opacity-20 disabled:cursor-not-allowed text-white transition-colors"
            >
          ...
          {form.imagenes.length > 1 && (
            <div className="flex justify-center gap-2 py-4 px-4 shrink-0 overflow-x-auto" onClick={e => e.stopPropagation()}>
              {form.imagenes.map((img, i) => (
                <button
                  key={i}
                  onClick={() => setLightboxIndex(i)}
                  className={`w-10 h-10 sm:w-12 sm:h-12 shrink-0 rounded-md overflow-hidden border-2 transition-all ${
